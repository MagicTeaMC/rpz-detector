...
async def query_domain_async(loop, resolver_map, domain, target_ips, matching_domains, processed_count, total_domains, retry_delay, max_retries, output_file, write_counter, write_threshold):
    attempts = 0
                ...
                for record in response:
                    ...
                    if ip in target_ips:
                        ...
                        if write_counter[0] >= write_threshold:
                            write_to_file(output_file, matching_domains, write_counter)
                        return
            processed_count[0] += 1
            return
...
async def worker_async(loop, domains, resolver_map, target_ips, matching_domains, processed_count, total_domains, retry_delay, max_retries, output_file, write_counter, write_threshold):
    for domain in domains:
        await query_domain_async(loop, resolver_map, domain, target_ips, matching_domains, processed_count, total_domains, retry_delay, max_retries, output_file, write_counter, write_threshold)
...
async def query_domains_async(input_file, output_file, dns_servers, target_ips, retry_delay, max_retries, num_tasks):
    ...
    chunk_size = (total_domains + num_tasks - 1) // num_tasks
    tasks = []
    for i in range(0, total_domains, chunk_size):
        chunk = domains[i:i + chunk_size]
        tasks.append(worker_async(loop, chunk, resolver_map, target_ips, matching_domains, processed_count, total_domains, retry_delay, max_retries, output_file, write_counter, write_threshold))
